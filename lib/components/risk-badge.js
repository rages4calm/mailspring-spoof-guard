...
var SEVERITY_COLOR = {
  critical: '#d92d20', high: '#e8590c', medium: '#b9810a', low: '#6b7280', info: '#6b7280',
};
...
class SpoofGuardBadge extends React.Component {
  ...
  _authChip(label, value) {
    var ok = value === 'pass';
    var color = ok ? '#2e9e5b' : (value === 'fail' ? '#d92d20' : '#9aa0a6');
  ...
  _renderAuthLine(d) {
    ...
    return h('div', { className: 'sg-auth-row', key: 'auth' },
      ...
      d.auth.arcPass ? h('span', { className: 'sg-auth-chip', style: { color: '#1f80c4' } }, 'ARC ✓') : null,
      d.auth.trusted ? h('span', { className: 'sg-auth-chip', style: { color: '#2e9e5b' } }, 'aligned ✓') : null
    );
  }
  ...
  _renderDetails() {
    ...
    if (d.blocked) {
      rows.push(h('div', { className: 'sg-note', key: 'blocked', style: { color: '#d92d20' } },
      ...
      rows.push(h('div', { className: 'sg-reasons', key: 'reasons' },
        r.signals.map(function (s, i) {
          return h('div', { className: 'sg-reason', key: i },
            h('span', { className: 'sg-dot', style: { background: SEVERITY_COLOR[s.severity] || '#6b7280' } }),
