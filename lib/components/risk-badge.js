...
// If mailsync never completes the raw fetch, fail over to the phase-1 result
// instead of showing "Checking SPF/DKIM/DMARC…" forever.
function withTimeout(promise, ms) {
  return new Promise(function (resolve, reject) {
    var t = setTimeout(function () { reject(new Error('raw fetch timed out')); }, ms);
    promise.then(
...
class SpoofGuardBadge extends React.Component {
  ...
  // True only when this message sits in the Sent folder AND its From address is
  // one of the user's own accounts — i.e. mail the user wrote, not a spoof.
  _sentByMe() {
    var msg = this.props.message;
    if (!msg) return false;
    try {
      var inSent = msg.folder && msg.folder.role === 'sent';
      var fromMe = typeof msg.isFromMe === 'function' ? msg.isFromMe() : false;
      return !!(inSent && fromMe);
    } catch (e) {
      return false;
    }
  }
  ...
  _run() {
    ...
    // Respect the master on/off switch; never analyze drafts.
    if (!config.get().enabled || msg.draft) {
  ...
  async _enrich() {
    ...
    try {
      ...
      await withTimeout(TaskQueue.waitForPerformRemote(task), 45000);
  ...
  render() {
    ...
    if (r.details && r.details.sentByMe) return null; // your own sent mail — no badge
