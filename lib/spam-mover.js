...
function optionsForAccount(accountId) {
  ...
  return {
    ...
    accountAliases: acct ? (acct.aliases || []) : [],
...
// Per-message flags: is this the user's own sent mail (Sent folder + own From)?
function messageFlags(message) {
  try {
    var inSent = message.folder && message.folder.role === 'sent';
    var fromMe = typeof message.isFromMe === 'function' ? message.isFromMe() : false;
    return { sentByMe: !!(inSent && fromMe) };
  } catch (e) {
    return { sentByMe: false };
  }
}

function optionsForMessage(message) {
  return Object.assign(optionsForAccount(message.accountId), messageFlags(message));
}
...
function decideAndMove(message, thread, result, requireAutoSpam) {
  try {
    ...
    // Never act on the user's own sent mail.
    if (result.details && result.details.sentByMe) return false;
    // For mail from the user's OWN address, only act on a raw-verified result —
    // the instant (pre-fetch) pass can't yet see the Received headers that
    // distinguish a genuine sent copy from an inbound spoof.
    if (result.details && result.details.fromMyAddress &&
        result.details.authStatus === 'pending') return false;
...
function fetchRawSource(message) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      // Time-limit the wait so one stuck fetch can't stall the deep-scan queue.
      var timer = setTimeout(function () { reject(new Error('raw fetch timed out')); }, 60000);
...
function processDeep() {
  ...
  DatabaseStore.find(Thread, message.threadId).then(function (thread) {
    ...
    return fetchRawSource(message).then(function (raw) {
      var opts = optionsForMessage(message);
...
function analyzeAndMaybeMove(message) {
  ...
  DatabaseStore.find(Thread, message.threadId).then(function (thread) {
    ...
    var result = analyzer.analyzeMessage(message, optionsForMessage(message));
    decideAndMove(message, thread, result, true);
