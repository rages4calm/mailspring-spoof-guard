...
function walkEntity(rawEntity, acc, depth) {
  ...
  // Parts referenced from the HTML via Content-ID (cid: images, logos) are
  // INLINE assets, not attachments — they go in a separate list so, e.g., a
  // newsletter's embedded SVG logo isn't treated like an attached .svg file.
  var filename = getParam(disposition, 'filename') || getParam(contentType, 'name');
  var contentId = getHeaderValues(headers, 'Content-ID')[0] || '';
  var isInline = /inline/i.test(disposition) || (!!contentId && !/attachment/i.test(disposition));
  ...
  if (contentType.indexOf('text/html') === 0) {
    ...
    (isInline ? acc.inlineFiles : acc.attachments).push(filename);
  }
}

// parseMessage(raw) -> { headers, text, html, attachments, inlineFiles }
function parseMessage(raw) {
  raw = String(raw || '');
  var top = splitHeaderAndBody(raw);
  var headers = parseHeaderBlock(top.headerText);

  var acc = { text: '', html: '', attachments: [], inlineFiles: [] };
