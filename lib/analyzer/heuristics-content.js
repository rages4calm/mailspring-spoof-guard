...
function contentSignals(parts, ctx) {
  ...
  if (/\b(scan (the )?(qr|code|barcode)|qr[\s-]?code)\b/i.test(haystack) &&
      (hasImage || countMatches(haystack, CREDENTIAL).length || countMatches(haystack, URGENCY).length)) {
    signals.push({
      id: 'qr_phishing',
  ...
  // Gather all URLs (anchors + bare text URLs) for structural checks.
  var allUrls = anchors.map(function (a) { return a.href; });
  var bare = (text.match(/\bhttps?:\/\/[^\s"'<>)]+/gi) || []);
  allUrls = allUrls.concat(bare);

  var sawIp = false, sawPuny = false, sawObf = false, sawShort = false;
  ...
  allUrls.forEach(function (u) {
    ...
    if (/@/.test(u.replace(/^[a-z]+:\/\//i, '')) && !sawObf) {
  ...
  // --- attachments ------------------------------------------------------------
  (parts.attachments || []).forEach(function (name) {
