...
function score(signals, auth) {
  ...
  signals.forEach(function (s) {
    ...
    if (s.severity === 'medium') hasMedium = true;
  });

  // A trusted, well-aligned message gets a small benefit of the doubt — but only
  // when nothing of note is present. We never discount genuine medium+ signals
  // (impersonation, advance-fee lures, etc.) just because auth/DKIM passed —
  // authenticated mail can still be phishing.
  if (trusted && !hasHigh && !hasCritical && !hasMedium) total = Math.max(0, total - 8);
