...
function analyzeCore(headers, parts, options) {
  ...
  // Mail the user actually SENT must never be flagged as a spoof of their own
  // domain (their stored sent copy has no Authentication-Results or server-side
  // DKIM-Signature, so it would otherwise look exactly like a spoof).
  // Identity: From is exactly the account address or one of its aliases.
  // Evidence it was sent (not received): the caller saw it in the Sent folder
  // (options.sentByMe), OR the raw source has ZERO Received headers — anything
  // that arrived over SMTP always carries at least one, only locally composed
  // mail has none. An inbound spoof using the user's exact address therefore
  // still gets flagged.
  var myEmails = [options.accountEmail].concat(options.accountAliases || [])
    .map(function (e) { return domains.extractEmail(String(e || '')) || String(e || '').toLowerCase(); })
    .filter(Boolean);
  var fromMyAddress = !!fromEmail && myEmails.indexOf(fromEmail) !== -1;
  var receivedCount = mime.getHeaderValues(headers, 'Received').length;
  var sentByMe = options.sentByMe === true ||
    (fromMyAddress && !options.authPending && receivedCount === 0);
  ...
  // Allow-listed senders and the user's own sent mail skip all heuristics and
  // score clean. Otherwise combine header + content signals, any externally-
  // supplied signals (e.g. online reputation), and a block-list override.
  var signals;
  if (allowListed || sentByMe) {
    signals = [];
  } else {
    signals = []
...
function analyzeMessage(message, options) {
  ...
  (message.files || []).forEach(function (f) {
    ...
    if (!name) return;
    // Mailspring File objects carry contentId when the file is an inline
    // (cid:-referenced) body asset rather than a real attachment.
    if (f.contentId) inlineFiles.push(name);
    else attachments.push(name);
  });
  var parts = {
    headers: headers,
    html: html,
