...
function headerSignals(headers, ctx) {
  ...
  // From: looks like your domain, but the message did not authenticate as your
  // domain (no aligned DKIM/SPF pass, or the return-path belongs elsewhere).
  if (claimsOurs && auth.present && !auth.trusted) {
    var returnElsewhere = returnDomain && !ours.some(function (d) {
      return domains.sameBaseDomain(returnDomain, d);
    });
    if (!auth.aligned || returnElsewhere) {
      signals.push({
        id: 'internal_spoof',
        label: 'Claims to be from your own domain but did not authenticate as it',
        severity: 'critical',
      });
    }
  }
